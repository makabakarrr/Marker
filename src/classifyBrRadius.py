...
def classifyArcByKmeans(circle_info):
    ...
    data = np.array(circle_info, dtype=np.float32)[:, 2]
...
def delBranch(edges, angle_thresh):
    ...
    cv2.imwrite("../images/process/"+type+"/approxResult/" + imgName + "_cnt_approx.png", sub_mask1)
    cv2.imwrite("../images/process/"+type+"/approxResult/" + imgName + "_cnt_break.png", sub_mask2)
    return sub_mask2


def filterArcByMaxK(circle_info, category_index):
    # 每一种半径类别里只保留k值最大的那个圆
    # 则每一个位置内有几个元素就代表该位置上有多少种半径，每一个元素均为该种半径内的k值最大的圆的参数信息[[pos1-[circle_x, circle_y,....]]]
    simply_circle_tree = []
    for c in range(np.max(category_index) + 1):
        cate_indexs = np.where(category_index == c)[0]  # 某一类的索引
        cate_circles = np.array(circle_info)[cate_indexs]   # 提取某一类内的圆参数
        ...
        cur_radius_k = np.argmax(np.array(k_list))
        cur_pos_circle.append(cur_radiu_circles[cur_radius_k])
        simply_circle_tree.append(cur_pos_circle)
    return simply_circle_tree
    # simplify_tree = []
...
def fillIntCircle(no_branch, other_circle_info):
    ...
    cv2.imwrite("../images/process/"+type+"/decode/" + imgName + "_coded_circles.png", canvas)
    return canvas
...
def decodedByGray(points, lp, side_length, img):
    ...
    for index in range(len(points)):
        ...
        if abs(p_gray - gray) > 5:
            ...
            int_str += '0'
    cv2.imwrite("../images/process/"+type+"/decode/" + imgName + '-pos.png', canvas)
    return int_str
...
def extractCircularEdges(sobel_thresh, side_length):
    ...
    cv2.imwrite("../images/process/"+type+"/decode/dec_edges/" + imgName + "_dec_edges.png", dec_edges)
    cv2.imwrite("../images/process/"+type+"/decode/dec_rect/" + imgName + "_dec_ellipse.png", dec_rect)
    return dec_edges
...
def classifyCircle(circle_tree):
    ...
    for position in circle_tree:
        ...
        print("len(position):", length)
        if length == 3:
            r1, r2, r3 = np.sort(r_list)
            print(r1, r2, r3, np.sort(r_list))
            ratio1 = r2 / r1
            ratio2 = r3 / r1
            print(ratio1, ratio2)
            # if abs(ratio1-2)<0.1 and abs(ratio2-3)<0.1:
            # if abs(ratio1-1.7)<0.1 and abs(ratio2-2.7)<0.1:
    ...
    for pos in template_center:
        ...
        for circle in pos:
            ...
            if np.isnan(s_x) or np.isnan(s_y) or np.isnan(s_a) or np.isnan(s_b):
                ...
                # cv2.imwrite("../images/process/"+type+"/gaussianBlur/" + imgName + '-nancanvas.png', nan_canvas)
                continue
            else:
...
edge_canvas = cv2.cvtColor(blur, cv2.COLOR_GRAY2BGR)
