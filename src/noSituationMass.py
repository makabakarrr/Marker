    ...
    for cnt in cnts0:
        if len(cnt) > 10:
            ...
            for index in range(1, len(approx) - 1):
                ...
                l_0 = p_0 - p_1
                l_1 = p_1 - p_2
                aa = np.round(np.dot(l_0, l_1) / (np.linalg.norm(l_0) * np.linalg.norm(l_1)), 6)
                A = 0 if aa>1.0 or aa<-1 else np.degrees(np.arccos(aa))
...
def extractCircularEdges(sobel_thresh, side_length, lp):
    center_x, center_y = lp
    height, width = edges.shape
    dec_edges = np.zeros((height, width), np.uint8)
    circular_range = np.zeros((height, width), np.uint8)
    dec_rect = np.zeros((height, width), np.uint8)
    dec_rect1 = np.zeros((height, width), np.uint8)
    ...
    circular_edges = cv2.bitwise_and(sobel_thresh, circular_range)
    contours, _ = cv2.findContours(circular_edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    for cnt in contours:
        if len(cnt) < 20:
            continue
        ...
        # box = np.int0(box)
        # cv2.drawContours(dec_rect, [box], 0, (255,255,255), -1)
        # x, y, w, h = cv2.boundingRect(cnt)
        # cv2.rectangle(dec_rect1, (x, y), (x + w, y + h), (255,255,255), -1)
    ...
    # dec_thresh = cv2.bitwise_and(cv2.bitwise_and(dec_rect, dec_rect1), circular_range)
...
def getCenterBySubEdges1(template_center, edges_points):
    ...
    for pos in template_center:
        ...
        for circle in pos:
            cur_edges = edges_points[int(circle[3])]
            cur_sub_edges = getSubEdgePoints(cur_edges)
            s_x, s_y, s_r = ransac_circle_fit(cur_sub_edges, 2000, 0.7*len(cur_sub_edges), 1)
...
for m in range(0, 30):
    ...
    # clahe = cv2.createCLAHE(1.5, (16,16))
    ...
    for n0 in start_circle_centers:
        cv2.putText(drawCircle, 'n0', (int(n0[0] + 50), int(n0[1] - 20)), cv2.FONT_HERSHEY_SIMPLEX, 1, (247, 162, 17),
                    2)
        ...
        sheet.cell(row=m + 2, column=13, value=round(angle, 4))
        # sheet.cell(row=m + 2, column=14, value=round(angle2, 4))
        # sheet.cell(row=m + 2, column=10, value=round(135-angle, 4))
        # sheet.cell(row=m + 2, column=7, value=round(135-angle, 4))

        ## 整数部分解码
        side_length = np.sqrt((n0[0]-n1[0])**2+(n0[1]-n1[1])**2)
        int_points = cvtCodePoints1(code_points, transform_matrix)
        int_value = getCodeVal(int_thresh, int_points)

        ## 小数部分解码
        dec_points = cvtCodePoints1(bandCenters, transform_matrix)
        # 提取出所有的圆环带轮廓

        circular_thresh = extractCircularEdges(sobel_thresh, side_length, lp)
        ...
        print(m, int(dec_value, 2), decode)
        # 计算中心坐标
        ...
        sheet.cell(row=m + 2, column=11, value=round(average_x, 4))
        sheet.cell(row=m + 2, column=12, value=round(average_y, 4))
        ...
        # sheet.cell(row=m + 2, column=6, value=round(average_y, 4))
        ## 计算角度
